<template>
  <cas :data="options" :loadData="loadOption" transfer @on-change="handleChange">
    <v-combobox v-model="selected" :box="box" :chips="chips" :clearable="clearable" :label="label" :outline="outline"
                :required="required" :rules="defaultRules" :single-line="single" multiple>
      <template v-slot:selection="{ item, parent, selected }">
        <v-chip v-if="multiple" close color="green" outline small @input="remove(item)" @click.stop="">
          {{ item.label }}&nbsp;
        </v-chip>
        <v-chip v-else small @click.stop="">{{ item }}</v-chip>
      </template>
    </v-combobox>
  </cas>
</template>
