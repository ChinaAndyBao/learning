<template>
  <v-card>
    ...
    <v-card-text class="px-3">
      <v-form ref="myBrandForm" v-model="valid">
        <v-container grid-list-md>
          <v-layout wrap>
            <v-flex class="md6">
              <v-text-field v-model="brand.name" :rules="nameRules" label="品牌名称" required/>
            </v-flex>
            <v-flex class="md6">
              <v-text-field v-model="brand.letter" :rules="letterRules" label="品牌首字母" required/>
            </v-flex>
            <v-flex>
              <v-cascader
          ...
          <v-layout row>
            ...
            <v-flex>
              <v-upload v-model="brand.image" :multiple="false" :pic-height="90" :pic-width="250"
                        need-signature url="/auth/ali/oss/signature"/>
            </v-flex>
          </v-layout>
        </v-container>
      </v-form>
