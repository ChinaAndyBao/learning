<template>
  <v-card>
    <v-card-title>
      ...
      <v-spacer/>
      <v-flex xs3>
        <v-text-field v-model.lazy="search" append-icon="search" hide-details label="输入关键字搜索"/>
      </v-flex>
    </v-card-title>
    <v-divider/>
    <v-data-table
    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="text-xs-center">
          <img v-if="props.item.image" :src="props.item.image" height="40" width="130">
    ...
    <!--弹出的对话框-->
    <v-dialog v-model="show" max-width="500" persistent scrollable>
      <brand-form :isEdit="isEdit" :oldBrand="oldBrand" @close="closeWindow"/>
    </v-dialog>
  </v-card>
</template>
