    ...
    <v-stepper-items>
      ...
      <v-stepper-content step="1">
        <v-flex class="xs10 mx-auto">
          <v-form ref="basic" v-model="valid">
            <!--商品分类-->
            <v-cascader
            ...
            <v-text-field v-model="goods.name" :counter="200" :rules="[v => !!v || '商品标题不能为空']" label="商品名称"
                          required/>
            <v-textarea v-model="goods.title" :counter="200" :rows="4" label="商品标题(会影响到搜索排名)"/>
          </v-form>
        </v-flex>
      </v-stepper-content>
      <!--2、商品描述-->
      <v-stepper-content step="2">
        <v-textarea v-model="goods.spuDetail.packingList" :counter="1000" :rows="3" hide-details label="包装清单"/>
        <v-textarea v-model="goods.spuDetail.afterService" :counter="1000" :rows="3" class="mb-4" hide-details
                    label="售后服务"/>
        <span class="subheading font-weight-thin">商品描述</span>
        <v-editor v-model="goods.spuDetail.description" class="mt-3" needSignature url="/auth/ali/oss/signature"/>
      </v-stepper-content>
      <!--3、规格参数-->
      <v-stepper-content step="3">
        <v-card>
          <v-flex class="headline mx-auto" xs3>通用规格属性</v-flex>
          <v-card-text>
            <v-container class="mt-0" fluid grid-list-md>

              <v-layout class="px-4" justify-space-between row wrap>
                <v-flex v-for="param in specs" :key="param.name" sm5 xs12>
                  <v-select v-if="param.ops && param.ops.length > 0" v-model="param.v" :items="param.ops"
                            :label="param.name"></v-select>
                  <v-text-field v-else v-model="param.v" :label="param.name" :suffix="param.unit || ''"
                  />
                </v-flex>
              </v-layout>
            </v-container>
        ...
        <v-divider></v-divider>
        <v-card>
          <v-flex class="headline mx-auto" xs3>特有用规格属性</v-flex>
          <v-card-text>
            <v-card v-for="spec in specialSpecs" :key="spec.name" flat>
              <!--特有参数的标题-->
              <v-flex class="subheading">{{ spec.name }}:</v-flex>
              <!--特有参数的待选项，需要判断是否有options，如果没有，展示文本框，让用户自己输入-->
              <v-flex>
                <v-card-text class="px-4">
                  <v-container v-if="spec.ops && spec.ops.length > 0" class="mt-0" fluid grid-list-md>
                    <v-layout class="px-4" justify-space-between row wrap>
                      <v-flex v-for="(o,i) in spec.ops" :key="o" sm3 xs12>
                        <v-checkbox v-model="spec.options" :label="o" :value="o" color="primary" hide-details/>
                      </v-flex>
                    </v-layout>
                  </v-container>

                  <v-container v-else class="mt-0" fluid grid-list-md>
                    <v-layout class="px-4" justify-space-between row wrap>
                      <v-flex v-for="i in spec.options.length+1" :key="i" class="row layout" sm6 xs12>
                        <v-text-field v-model="spec.options[i-1]" :placeholder="'新的' + spec.name + ':'" auto-grow
                                      class="flex xs10" hide-details single-line v-bind:value="i"/>
                        <v-btn v-if="i <= spec.options.length" icon @click="spec.options.splice(i-1,1)">
      ...
      <v-stepper-content step="4">
        <v-flex class="mx-auto">
          ...
          <v-card class="elevation-0">
            ...
            <v-data-table :headers="headers" :items="skus" class="elevation-0" hide-actions item-key="indexes">
              <template slot="items" slot-scope="props">
                <tr @click="props.expanded = !props.expanded">
                  <!--价格和库存展示为文本框-->
                  <td v-for="(v,k) in props.item" v-if="['price', 'stock'].includes(k)" :key="k"
                      class="text-xs-center">
                    <v-text-field v-model="props.item[k]" single-line @click.stop=""/>
                  </td>
                  <!--enable展示为checkbox-->
                  <td v-else-if="k === 'enable'" class="text-xs-center">
                    <v-checkbox v-model="props.item[k]"/>
                  </td>
                  <!--indexes和images不展示，其它展示为普通文本-->
                  <td v-else-if="k !== 'images' && k !== 'indexes'&& k !== 'id'" class="text-xs-center">{{ v.v }}</td>
                </tr>
              </template>
              <!--点击表格后展开-->
              <template slot="expand" slot-scope="props">
                <v-card class="elevation-2 flex xs11 mx-auto my-2">
                  <!--图片上传组件-->
                  <v-upload v-model="props.item.images" needSignature url="/auth/ali/oss/signature"/>
                </v-card>
              </template>
            </v-data-table>
          </v-card>
