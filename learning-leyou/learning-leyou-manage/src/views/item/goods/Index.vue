  ...
  <v-card class="mt-2">
    <v-toolbar class="elevation-0">
      ...
      <v-flex xs2>
        ...
        <v-btn-toggle v-model.lazy="saleable" mandatory>
          <v-btn large>
            全部
          </v-btn>
      ...
      <v-flex xs2>
        <v-cascader v-model="search.category" box
                    label="商品分类"
                    single
      ...
      <v-flex xs2>
        <!--品牌-->
        <v-autocomplete v-model="search.brandId"
                        :items="brandOptions"
                        autocomplete
                        box
                        chips
      ...
      <v-spacer></v-spacer>
      <v-flex xs2>
        <v-text-field v-model="search.id" append-icon="search" box class="mb-2"
                      hide-details label="商品编号" @keyup.enter="refresh"
    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-tooltip left>
            <v-btn slot="activator" icon @click="deleteGoods(props.item)">
          ...
          <v-tooltip left>
            <v-btn slot="activator" icon @click="updateSaleable(props.item.id, !props.item.saleable)">
              <i v-if="!props.item.saleable" class="el-icon-upload2"/>
    ...
    <v-dialog v-model="show" max-width="800" persistent scrollable>
      <v-card>
        ...
        <v-card-text class="px-0 py-0" style="height: 550px;">
          <goods-form ref="goodsForm" :is-edit="isEdit" :oldGoods="oldGoods" :step="step" @close="closeWindow"
                      @fallback="previous"/>
        </v-card-text>
        <!--底部按钮，用来操作步骤线-->
        <v-card-actions class="elevation-0">
          <v-flex class="xs3 mx-auto">
            <v-btn :disabled="step === 1" color="primary" @click="previous">上一步</v-btn>
