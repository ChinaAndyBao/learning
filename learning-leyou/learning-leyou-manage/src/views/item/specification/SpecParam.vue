    ...
    <v-dialog v-model="show" max-width="350px" persistent scrollable>
      <v-card>
        <v-card-text style="height: 300px;">
          <v-flex class="px-3">
            <v-text-field v-model="param.name" label="参数名称："/>
            <v-checkbox v-model="param.generic" color="primary" hide-details label="是否为通用属性"/>
            <v-checkbox v-model="param.numeric" color="primary" hide-details label="是否为数值类型"/>
            <v-text-field v-if="param.numeric" v-model="param.unit" label="数值单位："/>
            <v-checkbox v-model="param.searching" color="primary" hide-details label="是否用于搜索"/>
            <v-flex v-if="param.searching && param.numeric" class="px-2">
              搜索过滤区间：
              <v-layout v-for="(s,i) in param.segments" :key="i" row wrap>
                <v-flex xs5>
                  <v-text-field v-model="s[0]" hide-details prefix="From: " single-line/>
                </v-flex>
                <v-spacer/>
                <v-flex xs5>
                  <v-text-field v-model="s[1]" hide-details prefix="To: " single-line/>
                </v-flex>
                <v-flex xs1>
            ...
            <v-checkbox v-model="param.hasOption" color="primary" hide-details label="是否添加可选值"/>
            <v-flex v-if="param.hasOption" class="px-2">
              参数的可选值：
              <v-layout v-for="(s,i) in param.options" :key="i" row wrap>
                <v-flex xs8>
                  <v-text-field v-model="param.options[i]" hide-details single-line/>
                </v-flex>
                <v-spacer/>
                <v-flex xs1>
